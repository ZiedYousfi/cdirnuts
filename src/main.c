...
/**
 * Program entry point that parses command-line options, performs default initialization, and returns an exit status.
 *
 * Supported options:
 * - --help: prints usage and exits.
 * - --config <file>: loads configuration from <file>; if the file argument is missing or begins with "--", the program logs an error and exits with status 1.
 * A single non-option argument is treated as the project name; if none is provided, PROJECT_NAME is used.
 *
 * After parsing, the program logs a greeting and calls init_default_setup(getcwd(NULL, 0), projectName) before performing cleanup.
 *
 * @param argc Number of command-line arguments.
 * @param argv Array of command-line argument strings.
 * @returns 0 on success, 1 if a required --config filename is missing or invalid.
