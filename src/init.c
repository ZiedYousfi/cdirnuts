...
int init_default_setup(const char* parentDir, const char* projectName) {
  ...
  if (parentDir == NULL || projectName == NULL) {
    log_error("Parent directory or project name cannot be NULL.");
    return -1;
  }
  if (strlen(projectName) == 0) {
    log_error("Project name cannot be an empty string.");
    return -1;
  }
  if (strlen(parentDir) == 0) {
    log_error("Parent directory cannot be an empty string.");
    return -1;
  }
  ...
  if (projectDir == NULL) {
    log_error("Failed to construct the project directory path.");
    result = -1;
    goto cleanup_init_default_setup;
  }

  log_info("Project directory '%s' created successfully in '%s'.", projectName,
           parentDir);
  log_info("Project directory path: %s", projectDir);

  if (createDir("include", projectDir)) {
    log_error("Failed to create include directory.");
    result = -1;
    goto cleanup_init_default_setup;
  }
  ...
  if (srcPath == NULL) {
    log_error("Failed to construct src directory path.");
    result = -1;
    goto cleanup_init_default_setup;
  }
  ...
  if (mainFile == NULL) {
    log_error("Failed to create main.c file.");
    result = -1;
    goto cleanup_init_default_setup;
  }
  ...
  log_info("main.c file created and modified successfully.");
  ...
  if (gitignoreFile == NULL) {
    log_error("Failed to modify .gitignore file content.");
    result = -1;
    goto cleanup_init_default_setup;
  }

  log_info(".gitignore file created and modified successfully.");
  readmeFile = createFile("README.md", projectDir);
  if (readmeFile == NULL) {
    log_error("Failed to create README.md file.");
    result = -1;
    goto cleanup_init_default_setup;
  }
  ...
  if (readmeFile == NULL) {
    log_error("Failed to modify README.md file content.");
    result = -1;
    goto cleanup_init_default_setup;
  }
  log_info("README.md file created and modified successfully.");
  makefileFile = createFile("Makefile", projectDir);
  if (makefileFile == NULL) {
    log_error("Failed to create Makefile file.");
    result = -1;
    goto cleanup_init_default_setup;
  }
  makefileFile = modifyFileContent(
      ...
      "$(CFLAGS) -c src/main.c\n\nclean:\n\trm -f *.o main\n");
  if (makefileFile == NULL) {
    log_error("Failed to modify Makefile file content.");
    result = -1;
    goto cleanup_init_default_setup;
  }
  log_info("Makefile file created and modified successfully.");
